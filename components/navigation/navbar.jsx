...
const Navbar = () => {
  ...
  return (
    ...
    <nav className="  bg-gray-900 text-white fixed w-full z-20 top-0 start-0 border-b   border-gray-600">
      <NavbarAnimation>
        <Logo />
            ...
            {navData.map((item, index) => {
              return (
                <li key={index}>
                  <Link
                    href={item.path}
                    className="block py-2 px-3     rounded md:bg-transparent   md:p-0   text-gray-50 hover:font-semibold hover:text-gray-400"
                    onClick={closeNav}
                  >
                    {item.name}
                  </Link>
